"""
Exercise: Linear Regression using Adadelta
...
def draw_plot(x, y, theta_history):
    ...
    ax.set_title("Adadelta, iter={0}".format(len(theta_history)), fontdict={"fontsize": 12})
    ax.tick_params(axis="both", direction="in", which="major", labelsize=8)

    # save the figure
    plt.tight_layout()
    plt.savefig("gradient-descent_Adadelta.png", dpi=PLOT_DPI, format=PLOT_FORMAT, bbox_inches='tight', pad_inches=0.05)
    plt.close()
