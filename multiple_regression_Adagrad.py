"""
Exercise: Multiple Regression using Adagrad
...
def gradient_descent(x, y, theta, rate_learning):
    ...
    while error_diff > error_stop:
        ...
        # calculate gradient
        # use Adagrad, the rate_learning should be at least larger, e.g., rate_learning = 1.0
        fx = f_theta(x, theta)
        grad0 = np.sum(fx - y)
        grad1 = np.sum((fx - y) * x1)
        grad2 = np.sum((fx - y) * x2)
        theta0_grad += np.sqrt(np.power(grad0, 2))
        theta1_grad += np.sqrt(np.power(grad1, 2))
        theta2_grad += np.sqrt(np.power(grad2, 2))

        # update theta
        theta0 = theta[0] - (rate_learning / np.sqrt(theta0_grad)) * grad0
        theta1 = theta[1] - (rate_learning / np.sqrt(theta1_grad)) * grad1
        theta2 = theta[2] - (rate_learning / np.sqrt(theta2_grad)) * grad2
...
def draw_plot(x, y, theta, error_history):
    ...
    ax.set_title("Mean Square Error (MSE), Adagrad, iter={0}".format(len(error_history)), fontdict={"fontsize": 12})
    ...
    # save image
    plt.tight_layout()
    plt.savefig("multiple-regression_MSE-Adagrad.png", dpi=PLOT_DPI, format=PLOT_FORMAT)
    plt.close()
